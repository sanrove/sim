...
export const hubspotAllTriggerOptions = [
  ...
  {
    label: "Conversation Property Changed",
 ...
 */
export function hubspotSetupInstructions(
  eventType: string,
  ...
  const instructions = [
    ...
    "<strong>Step 3: Configure OAuth Settings</strong><br/>After creating your app via CLI, configure it to add the OAuth Redirect URL: <code>https://www.ethana.ai/api/auth/oauth2/callback/hubspot</code>. Then retrieve your <strong>Client ID</strong> and <strong>Client Secret</strong> from your app configuration and enter them in the fields above.",
    ...
    '<strong>Step 6: Save Configuration in AgentBuilder</strong><br/>Click the <strong>"Save Configuration"</strong> button below. This will generate your unique webhook URL.',
    ...
    "<strong>Step 8: Test Your Webhook</strong><br/>Create or modify a contact in HubSpot to trigger the webhook. Check your workflow execution logs in AgentBuilder to verify it's working.",
  ];
  ...
  return instructions
    .map((instruction, index) => `<div class="mb-3">${instruction}</div>`)
    .join("");
}
...
function buildBaseHubSpotOutputs(): Record<string, TriggerOutput> {
  return {
    payload: {
      type: "json",
      description:
        "Full webhook payload array from HubSpot containing event details",
    },
    provider: {
      type: "string",
    ...
    providerConfig: {
      ...
      propertyName: {
        type: "string",
        description:
          "Optional property name filter (for property change triggers)",
      },
    },
  } as any;
}
 ...
 */
export function buildContactPrivacyDeletedOutputs(): Record<
  string,
  TriggerOutput
> {
  return buildBaseHubSpotOutputs();
 ...
 */
export function buildContactPropertyChangedOutputs(): Record<
  string,
  TriggerOutput
> {
  return buildBaseHubSpotOutputs();
 ...
 */
export function buildCompanyPropertyChangedOutputs(): Record<
  string,
  TriggerOutput
> {
  return buildBaseHubSpotOutputs();
 ...
 */
export function buildConversationCreationOutputs(): Record<
  string,
  TriggerOutput
> {
  return buildBaseHubSpotOutputs();
 ...
 */
export function buildConversationDeletionOutputs(): Record<
  string,
  TriggerOutput
> {
  return buildBaseHubSpotOutputs();
 ...
 */
export function buildConversationNewMessageOutputs(): Record<
  string,
  TriggerOutput
> {
  return buildBaseHubSpotOutputs();
 ...
 */
export function buildConversationPrivacyDeletionOutputs(): Record<
  string,
  TriggerOutput
> {
  return buildBaseHubSpotOutputs();
 ...
 */
export function buildConversationPropertyChangedOutputs(): Record<
  string,
  TriggerOutput
> {
  return buildBaseHubSpotOutputs();
 ...
 */
export function buildDealPropertyChangedOutputs(): Record<
  string,
  TriggerOutput
> {
  return buildBaseHubSpotOutputs();
 ...
 */
export function buildTicketPropertyChangedOutputs(): Record<
  string,
  TriggerOutput
> {
  return buildBaseHubSpotOutputs();
 ...
 */
export function isHubSpotContactEventMatch(
  triggerId: string,
  eventType: string
): boolean {
