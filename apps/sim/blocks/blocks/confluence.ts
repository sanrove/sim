...
export const ConfluenceBlock: BlockConfig<ConfluenceResponse> = {
  ...
  docsLink: "https://docs.ethana.ai/tools/confluence",
  category: "tools",
  bgColor: "#E0E0E0",
  icon: ConfluenceIcon,
  subBlocks: [
    ...
    config: {
      ...
      params: (params) => {
        ...
        if (requiresPageId.includes(operation) && !effectivePageId) {
          throw new Error(
            "Page ID is required. Please select a page or enter a page ID manually."
          );
        }
  ...
  inputs: {
    ...
    attachmentFileName: {
      type: "string",
      description: "Custom file name for attachment",
    },
    attachmentComment: {
      type: "string",
