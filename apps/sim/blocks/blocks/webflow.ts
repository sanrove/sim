...
export const WebflowBlock: BlockConfig<WebflowResponse> = {
  ...
  docsLink: "https://docs.ethana.ai/tools/webflow",
  category: "tools",
  triggerAllowed: true,
  bgColor: "#E0E0E0",
  icon: WebflowIcon,
      ...
      params: (params) => {
        ...
        switch (params.operation) {
          ...
          case "delete":
            if (!effectiveItemId) {
              throw new Error(
                `Item ID is required for ${params.operation} operation`
              );
            }
  ...
  inputs: {
    ...
    collectionId: {
      type: "string",
      description: "Webflow collection identifier",
    },
    manualCollectionId: {
      type: "string",
      description: "Manual collection identifier",
    },
  ...
  outputs: {
    ...
    item: {
      type: "json",
      description: "Single item data (get/create/update operations)",
    },
    success: {
      type: "boolean",
      description: "Operation success status (delete operation)",
    },
    ...
    workspaceId: {
      type: "string",
      description: "Workspace ID where event occurred",
    },
    collectionId: {
      type: "string",
      description: "Collection ID (for collection events)",
    },
    payload: {
      type: "json",
      description: "Event payload data (item data for collection events)",
    },
    name: { type: "string", description: "Form name (for form submissions)" },
    id: { type: "string", description: "Submission ID (for form submissions)" },
    submittedAt: {
      type: "string",
      description: "Submission timestamp (for form submissions)",
    },
    data: {
      type: "json",
      description: "Form field data (for form submissions)",
    },
    schema: { type: "json", description: "Form schema (for form submissions)" },
    formElementId: {
      type: "string",
      description: "Form element ID (for form submissions)",
    },
  },
  triggers: {
    enabled: true,
