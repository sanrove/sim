...
export const SharepointBlock: BlockConfig<SharepointResponse> = {
  ...
  docsLink: "https://docs.ethana.ai/tools/sharepoint",
  category: "tools",
  bgColor: "#E0E0E0",
  ...
  subBlocks: [
...

    {
      ...
      condition: {
        field: "operation",
        value: ["read_list", "update_list", "add_list_items"],
      },
    },
...

    {
      ...
      condition: {
        field: "operation",
        value: ["update_list", "add_list_items"],
      },
    },
    ...
    config: {
      tool: (params) => {
        switch (params.operation) {
          ...
          default:
            throw new Error(
              `Invalid Sharepoint operation: ${params.operation}`
            );
        }
      },
      params: (params) => {
        const { credential, siteSelector, manualSiteId, mimeType, ...rest } =
          params;
        ...
        if (
          others.operation === "update_list" ||
          others.operation === "add_list_items"
        ) {
          try {
        ...
        const baseParams = {
          ...
          pageSize: others.pageSize
            ? Number.parseInt(others.pageSize as string, 10)
            : undefined,
          mimeType: mimeType,
          ...others,
          itemId: sanitizedItemId,
  ...
  outputs: {
    ...
    list: {
      type: "json",
      description:
        "SharePoint list object (id, displayName, name, webUrl, etc.)",
    },
    item: {
      type: "json",
