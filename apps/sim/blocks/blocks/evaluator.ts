...
export const generateEvaluatorPrompt = (
  ...
  const metricsDescription = validMetrics
    .map(
      (metric) =>
        `"${metric.name}" (${metric.range.min}-${metric.range.max}): ${
          metric.description || ""
  ...
  const exampleOutput = validMetrics.reduce((acc, metric) => {
  ...
  validMetrics.forEach((metric) => {
    ...
    if (metric?.name) {
      properties[metric.name.toLowerCase()] = {
        type: "number",
        description: `${metric.description || ""} (Score between ${
          metric.range?.min ?? 0
        }-${metric.range?.max ?? "N/A"})`, // Safely access range
      };
    } else {
      logger.warn(
...
export const EvaluatorBlock: BlockConfig<EvaluatorResponse> = {
  ...
  docsLink: "https://docs.ethana.ai/blocks/evaluator",
  category: "tools",
  bgColor: "#4D5FFF",
  icon: ChartBarIcon,
  subBlocks: [
