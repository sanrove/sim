...
export const MicrosoftTeamsBlock: BlockConfig<MicrosoftTeamsResponse> = {
  ...
  docsLink: "https://docs.ethana.ai/tools/microsoft_teams",
  category: "tools",
  triggerAllowed: true,
  bgColor: "#E0E0E0",
  ...
  tools: {
    ...
    config: {
      ...
      params: (params) => {
        ...
        if (
          (operation === "read_chat" || operation === "read_channel") &&
          includeAttachments
        ) {
        ...
        if (
          fileParam &&
          (operation === "write_chat" || operation === "write_channel")
        ) {
        ...
        if (
          ...
          if (!effectiveChatId) {
            throw new Error(
              "Chat ID is required. Please select a chat or enter a chat ID."
            );
          }
        ...
        if (operation === "list_channel_members") {
          if (!effectiveTeamId) {
            throw new Error(
              "Team ID is required for channel member operations."
            );
          }
          if (!effectiveChannelId) {
            throw new Error(
              "Channel ID is required for channel member operations."
            );
          }
          return {
            ...baseParams,
  ...
  inputs: {
    ...
    messageId: {
      type: "string",
      description:
        "Message identifier for update/delete/reply/reaction operations",
    },
    ...
    manualChannelId: {
      type: "string",
      description: "Manual channel identifier",
    },
  ...
  outputs: {
    content: {
      type: "string",
      description: "Formatted message content from chat/channel",
    },
    metadata: {
      type: "json",
      description: "Message metadata with full details",
    },
    messageCount: {
      type: "number",
    ...
    attachmentTypes: {
      type: "json",
      description: "Array of attachment content types",
    },
    attachments: {
      type: "array",
      description: "Downloaded message attachments",
    },
    updatedContent: {
      type: "boolean",
    ...
    deleted: {
      type: "boolean",
      description: "Whether message was successfully deleted",
    },
    messageId: {
      type: "string",
      description: "ID of the created/sent/deleted message",
    },
    createdTime: {
      type: "string",
    ...
    messageTimestamp: {
      type: "string",
      description: "Individual message timestamp",
    },
    messageType: {
      type: "string",
    ...
    reactionType: {
      type: "string",
      description: "Emoji reaction that was added/removed",
    },
    success: {
      type: "boolean",
    ...
    members: {
      type: "json",
      description: "Array of team/channel member objects",
    },
