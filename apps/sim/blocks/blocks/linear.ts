...
export const LinearBlock: BlockConfig<LinearResponse> = {
  ...
  docsLink: "https://docs.ethana.ai/tools/linear",
  category: "tools",
  icon: LinearIcon,
  bgColor: "#5E6AD2",
  subBlocks: [
      ...
      params: (params) => {
        ...
        switch (params.operation) {
          ...
          case "linear_create_issue_relation":
            if (
              !params.issueId?.trim() ||
              !params.relatedIssueId?.trim() ||
              !params.relationType
            ) {
              throw new Error(
                "Issue ID, related issue ID, and relation type are required."
              );
            }
            return {
              ...baseParams,
          ...
          case "linear_create_customer_request":
            ...
            return {
              ...
              priority:
                params.priority !== undefined ? Number(params.priority) : 0,
          ...
          case "linear_update_customer":
            ...
            return {
              ...
              externalIds: params.customerExternalIds?.trim()
                ? params.customerExternalIds
                    .split(",")
                    .map((id: string) => id.trim())
                : undefined,
              logoUrl: params.customerLogoUrl?.trim() || undefined,
              ownerId: params.customerOwnerId?.trim() || undefined,
              revenue: params.customerRevenue
                ? Number(params.customerRevenue)
                : undefined,
              size: params.customerSize
                ? Number(params.customerSize)
                : undefined,
          ...
          case "linear_merge_customers":
            if (
              !params.sourceCustomerId?.trim() ||
              !params.targetCustomerId?.trim()
            ) {
              throw new Error(
                "Both source and target customer IDs are required."
              );
            }
            return {
              ...baseParams,
          ...
          case "linear_create_customer_status":
            ...
            return {
              ...
              displayName:
                params.statusDisplayName?.trim() || params.statusName.trim(),
          ...
          case "linear_create_customer_tier":
            ...
            return {
              ...
              displayName:
                params.tierDisplayName?.trim() || params.tierName.trim(),
              color: params.statusColor.trim(),
          ...
          case "linear_create_project_milestone":
            if (
              !params.projectIdForMilestone?.trim() ||
              !params.milestoneName?.trim()
            ) {
  ...
  inputs: {
    ...
    favoriteTargetId: {
      type: "string",
      description: "Favorite target identifier",
    },
    ...
    customerDomains: {
      type: "string",
      description: "Customer domains (comma-separated)",
    },
    customerId: { type: "string", description: "Customer identifier" },
    customerNeedId: {
      type: "string",
      description: "Customer request identifier",
    },
    requestBody: {
      type: "string",
    ...
    linkedIssueId: {
      type: "string",
      description: "Issue ID to link to customer request",
    },
    // New customer management inputs
    customerIdTarget: {
      type: "string",
      description: "Customer ID for operations",
    },
    sourceCustomerId: {
      type: "string",
      description: "Source customer ID for merge",
    },
    targetCustomerId: {
      type: "string",
      description: "Target customer ID for merge",
    },
    customerExternalIds: {
      type: "string",
      description: "Customer external IDs (comma-separated)",
    },
    ...
    projectLabelIsGroup: {
      type: "string",
      description: "Whether label is a group (true/false)",
    },
    ...
    projectIdForMilestone: {
      type: "string",
      description: "Project ID for milestone operations",
    },
    ...
    projectStatusId: {
      type: "string",
      description: "Project status identifier",
    },
  ...
  outputs: {
    ...
    pageInfo: {
      type: "json",
      description:
        "Pagination information (hasNextPage, endCursor) for list operations",
    },
    // Success indicators
    ...
    action: {
      type: "string",
      description: "Webhook action (create, update, remove)",
    },
    type: {
      type: "string",
    ...
    webhookTimestamp: {
      type: "number",
      description: "Webhook timestamp in milliseconds",
    },
