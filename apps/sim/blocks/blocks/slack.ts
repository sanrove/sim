...
export const SlackBlock: BlockConfig<SlackResponse> = {
  ...
  docsLink: "https://docs.ethana.ai/tools/slack",
  category: "tools",
  bgColor: "#611f69",
  icon: SlackIcon,
  triggerAllowed: true,
    ...
    config: {
      ...
      params: (params) => {
        ...
        if (isDM && dmSupportedOperations.includes(operation)) {
          ...
          !effectiveChannel &&
          !noChannelOperations.includes(operation)
        ) {
        ...
        if (authMethod === "bot_token") {
          ...
          if (!credential) {
            throw new Error(
              "Slack account credential is required when using Sim Bot"
            );
          }
        ...
        switch (operation) {
          ...
          case "canvas":
            if (!title || !content) {
              throw new Error(
                "Title and content are required for canvas operation"
              );
            }
          ...
          case "list_channels": {
            ...
            baseParams.limit = channelLimit
              ? Number.parseInt(channelLimit, 10)
              : 100;
            break;
          }

          case "list_members": {
            baseParams.limit = memberLimit
              ? Number.parseInt(memberLimit, 10)
              : 100;
            break;
          }
          ...
          case "update":
            if (!updateTimestamp || !updateText) {
              throw new Error(
                "Timestamp and text are required for update operation"
              );
            }
          ...
          case "react":
            if (!reactionTimestamp || !emojiName) {
              throw new Error(
                "Timestamp and emoji name are required for reaction operation"
              );
            }
  ...
  inputs: {
    ...
    dmUserId: {
      type: "string",
      description: "User ID for DM recipient (selector)",
    },
    manualDmUserId: {
      type: "string",
      description: "User ID for DM recipient (manual input)",
    },
    ...
    updateTimestamp: {
      type: "string",
      description: "Message timestamp for update",
    },
    updateText: { type: "string", description: "New text for update" },
    deleteTimestamp: {
      type: "string",
      description: "Message timestamp for delete",
    },
    reactionTimestamp: {
      type: "string",
      description: "Message timestamp for reaction",
    },
    ...
    includePrivate: {
      type: "string",
      description: "Include private channels (true/false)",
    },
    channelLimit: {
      type: "string",
    ...
    includeDeleted: {
      type: "string",
      description: "Include deactivated users (true/false)",
    },
    userLimit: {
      type: "string",
  ...
  outputs: {
    ...
    ts: {
      type: "string",
      description: "Message timestamp returned by Slack API",
    },
    channel: {
      type: "string",
      description: "Channel identifier where message was sent",
    },
    fileCount: {
      type: "number",
    ...
    canvas_id: {
      type: "string",
      description: "Canvas identifier for created canvases",
    },
    title: { type: "string", description: "Canvas title" },
    ...
    count: {
      type: "number",
      description:
        "Total number of items returned (channels, members, or users)",
    },
    ...
    content: {
      type: "string",
      description: "Success message for update operation",
    },
    metadata: {
      type: "json",
      description:
        "Updated message metadata (legacy, use message object instead)",
    },

    // Trigger outputs (when used as webhook trigger)
    event_type: {
      type: "string",
      description: "Type of Slack event that triggered the workflow",
    },
    channel_name: {
      type: "string",
    ...
    event_id: {
      type: "string",
      description: "Unique event identifier for the trigger",
    },
  },
