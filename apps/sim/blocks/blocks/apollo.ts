...
export const ApolloBlock: BlockConfig<ApolloResponse> = {
  ...
  docsLink: "https://docs.ethana.ai/tools/apollo",
  category: "tools",
  bgColor: "#EBF212",
  icon: ApolloIcon,
  subBlocks: [
    ...
    config: {
      ...
      params: (params) => {
        ...
        try {
          ...
          if (
            rest.person_locations &&
            typeof rest.person_locations === "string"
          ) {
            parsedParams.person_locations = JSON.parse(rest.person_locations);
          }
          if (
            rest.person_seniorities &&
            typeof rest.person_seniorities === "string"
          ) {
            parsedParams.person_seniorities = JSON.parse(
              rest.person_seniorities
            );
          }
          if (
            rest.organization_names &&
            typeof rest.organization_names === "string"
          ) {
            parsedParams.organization_names = JSON.parse(
              rest.organization_names
            );
          }
          if (
            rest.organization_locations &&
            typeof rest.organization_locations === "string"
          ) {
            parsedParams.organization_locations = JSON.parse(
              rest.organization_locations
            );
          }
          if (
          ...
          if (
            ...
            parsedParams.q_organization_keyword_tags = JSON.parse(
              rest.q_organization_keyword_tags
            );
          }
          if (
            rest.contact_stage_ids &&
            typeof rest.contact_stage_ids === "string"
          ) {
            parsedParams.contact_stage_ids = JSON.parse(rest.contact_stage_ids);
          }
          if (
            rest.account_stage_ids &&
            typeof rest.account_stage_ids === "string"
          ) {
        ...
        if (
          params.operation === "account_create" ||
          params.operation === "account_update"
        ) {
        ...
        if (parsedParams.per_page)
          parsedParams.per_page = Number(parsedParams.per_page);

        // Convert amount to number if provided
        if (parsedParams.amount)
          parsedParams.amount = Number(parsedParams.amount);
  ...
  outputs: {
    ...
    output: {
      type: "json",
      description: "Output data from the Apollo operation",
    },
  },
};
