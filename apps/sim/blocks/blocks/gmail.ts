...
export const GmailBlock: BlockConfig<GmailToolResponse> = {
  ...
  docsLink: "https://docs.ethana.ai/tools/gmail",
  category: "tools",
  bgColor: "#E0E0E0",
  icon: GmailIcon,
  triggerAllowed: true,
  subBlocks: [
    ...
    },
    {
      ...
      type: "short-input",
      placeholder:
        'Gmail message ID (not RFC Message-ID) - use the "id" field from results',
      condition: { field: "operation", value: ["send_gmail", "draft_gmail"] },
      mode: "advanced",
      required: false,
    },
    ...
    // Add/Remove Label - Message ID field
    {
      ...
      condition: {
        field: "operation",
        value: ["add_label_gmail", "remove_label_gmail"],
      },
      required: true,
    },
  ...
  tools: {
    ...
    config: {
      ...
      params: (params) => {
        ...
        if (
          ["add_label_gmail", "remove_label_gmail"].includes(rest.operation)
        ) {
  ...
  inputs: {
    ...
    replyToMessageId: {
      type: "string",
      description:
        'Gmail message ID to reply to (use "id" field from results, not "messageId")',
    },
    cc: { type: "string", description: "CC recipients (comma-separated)" },
    bcc: { type: "string", description: "BCC recipients (comma-separated)" },
    attachments: {
      type: "array",
      description: "Files to attach (UserFile array)",
    },
    ...
    readMessageId: {
      type: "string",
      description: "Message identifier for reading specific email",
    },
    ...
    manualDestinationLabel: {
      type: "string",
      description: "Manual destination label ID",
    },
    sourceLabel: { type: "string", description: "Source label ID to remove" },
    manualSourceLabel: {
      type: "string",
      description: "Manual source label ID",
    },
    ...
    labelActionMessageId: {
      type: "string",
      description: "Message ID for label actions",
    },
    ...
    labelIds: {
      type: "string",
      description: "Label IDs for add/remove operations",
    },
  },
  outputs: {
    // Tool outputs
