...
export const SupabaseBlock: BlockConfig<SupabaseResponse> = {
  ...
  docsLink: "https://docs.ethana.ai/tools/supabase",
  category: "tools",
  bgColor: "#1C1C1C",
  icon: SupabaseIcon,
  subBlocks: [
    {
      ...
      options: [
        ...
        {
          label: "Storage: Create Signed URL",
...
Return ONLY the PostgREST filter expression - no explanations, no markdown, no extra text.`,
  ...
  tools: {
    ...
    config: {
      ...
      params: (params) => {
        ...
        if (
          allowedMimeTypes &&
          typeof allowedMimeTypes === "string" &&
          allowedMimeTypes.trim()
        ) {
          try {
        ...
        const parsedIsPublic =
          rest.isPublic === "true" || rest.isPublic === true;
  ...
  inputs: {
    ...
    countType: {
      type: "string",
      description: "Count type: exact, planned, or estimated",
    },
    ...
    fileName: {
      type: "string",
      description: "Optional filename override for downloaded file",
    },
    upsert: {
      type: "boolean",
    ...
    toPath: {
      type: "string",
      description: "Destination file path for move/copy",
    },
    ...
    expiresIn: {
      type: "number",
      description: "Expiration time in seconds for signed URL",
    },
    isPublic: {
      type: "boolean",
      description: "Whether bucket should be public",
    },
    fileSizeLimit: {
      type: "number",
