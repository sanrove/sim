...
export const SttBlock: BlockConfig<SttBlockResponse> = {
  ...
  docsLink: "https://docs.ethana.ai/tools/stt",
  category: "tools",
  bgColor: "#181C1E",
  icon: STTIcon,
  ...
  subBlocks: [
    ...
    // Audio/Video file upload (basic mode)
    {
      ...
      required: true,
      acceptedTypes:
        ".mp3,.m4a,.wav,.webm,.ogg,.flac,.aac,.opus,.mp4,.mov,.avi,.mkv",
    },
  ...
  inputs: {
    provider: {
      type: "string",
      description:
        "STT provider (whisper, deepgram, elevenlabs, assemblyai, gemini)",
    },
    ...
    audioFileReference: {
      type: "json",
      description: "Audio/video file reference",
    },
    audioUrl: { type: "string", description: "Audio/video URL" },
    language: { type: "string", description: "Language code or auto" },
    timestamps: {
      type: "string",
      description: "Timestamp granularity (none, sentence, word)",
    },
    diarization: { type: "boolean", description: "Enable speaker diarization" },
    translateToEnglish: {
      type: "boolean",
      description: "Translate to English (Whisper only)",
    },
    sentiment: {
      type: "boolean",
      description: "Enable sentiment analysis (AssemblyAI only)",
    },
    entityDetection: {
      type: "boolean",
      description: "Enable entity detection (AssemblyAI only)",
    },
    piiRedaction: {
      type: "boolean",
      description: "Enable PII redaction (AssemblyAI only)",
    },
    summarization: {
      type: "boolean",
      description: "Enable auto summarization (AssemblyAI only)",
    },
  },
  ...
  outputs: {
    ...
    confidence: {
      ...
      condition: {
        field: "provider",
        value: ["deepgram", "assemblyai", "gemini"],
      },
    },
    sentiment: {
      type: "array",
