...
export const ApiTriggerBlock: BlockConfig = {
  ...
  name: "API (Legacy)",
  description:
    "Legacy block for exposing HTTP API endpoint. Prefer Start block.",
  longDescription:
  ...
  - In production, the curl would come in as e.g. curl -X POST -H "X-API-Key: $ETHANA_AGENT_BUILDER_KEY" -H "Content-Type: application/json" -d '{"paramName":"example"}' https://www.staging.ethana.ai/api/workflows/9e7e4f26-fc5e-4659-b270-7ea474b14f4a/execute -- If user asks to test via API, you might need to clarify the API key.
  `,
  category: "triggers",
