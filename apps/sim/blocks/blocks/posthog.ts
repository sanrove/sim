...
export const PostHogBlock: BlockConfig<PostHogResponse> = {
  ...
  docsLink: "https://docs.ethana.ai/tools/posthog",
  category: "tools",
  bgColor: "#E0E0E0",
  icon: PosthogIcon,
  subBlocks: [
    ...
    // Property type field
    {
      ...
      condition: {
        field: "operation",
        value: "posthog_update_property_definition",
      },
    },
  ...
  tools: {
    ...
    config: {
      tool: (params) => {
        ...
        if (params.rolloutPercentage)
          params.rolloutPercentage = Number(params.rolloutPercentage);

        // Map projectIdParam to projectId for get_project operation
        if (
          params.operation === "posthog_get_project" &&
        ...
        if (
          flagOps.includes(params.operation as string) &&
        ...
        if (
          params.operation === "posthog_create_cohort" &&
        ...
        if (
          params.operation === "posthog_create_annotation" &&
          params.dateMarker
        ) {
        ...
        if (
          params.operation === "posthog_update_property_definition" &&
          params.propertyType
        ) {
  ...
  inputs: {
    ...
    projectApiKey: {
      type: "string",
      description: "Project API key for public endpoints",
    },
    personalApiKey: {
      type: "string",
      description: "Personal API key for private endpoints",
    },
    ...
    featureFlagKey: {
      type: "string",
      description: "Feature flag key for experiment",
    },
    parameters: {
      type: "string",
      description: "Experiment parameters as JSON",
    },
    questions: {
      type: "string",
      description: "Survey questions as JSON array",
    },
