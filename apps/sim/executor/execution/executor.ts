...
export class DAGExecutor {
  ...
  async execute(
    ...
    const dag = this.dagBuilder.build(
      this.workflow,
      triggerBlockId,
      savedIncomingEdges
    );
    const { context, state } = this.createExecutionContext(
      workflowId,
      triggerBlockId
    );
  ...
  private createExecutionContext(
    ...
    logger.info("[Executor] Creating execution context", {
      workflowId,
      hasContextExtensionsTenantId: !!this.contextExtensions.tenantId,
      contextExtensionsTenantId: this.contextExtensions.tenantId || "none",
    });
    ...
    const context: ExecutionContext = {
      ...
      tenantId: this.contextExtensions.tenantId,
      ...
      loopExecutions: snapshotState?.loopExecutions
        ? new Map(
            Object.entries(snapshotState.loopExecutions).map(
              ([loopId, scope]) => [
                loopId,
                {
                  ...scope,
      ...
      parallelExecutions: snapshotState?.parallelExecutions
        ? new Map(
            Object.entries(snapshotState.parallelExecutions).map(
              ([parallelId, scope]) => [
                parallelId,
                {
                  ...scope,
                  branchOutputs: scope.branchOutputs
                    ? new Map(
                        Object.entries(scope.branchOutputs).map(([k, v]) => [
                          Number(k),
                          v,
                        ])
                      )
    ...
    if (this.contextExtensions.remainingEdges) {
      (context.metadata as any).remainingEdges =
        this.contextExtensions.remainingEdges;
    ...
    if (this.contextExtensions.resumePendingQueue?.length) {
      context.metadata.pendingBlocks = [
        ...this.contextExtensions.resumePendingQueue,
      ];
  ...
  private initializeStarterBlock(
    ...
    let startResolution: ReturnType<typeof resolveExecutorStartBlock> | null =
      null;

    if (triggerBlockId) {
      const triggerBlock = this.workflow.blocks.find(
        (b) => b.id === triggerBlockId
      );
      if (!triggerBlock) {
