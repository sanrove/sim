...
export class DeployWorkflowClientTool extends BaseClientTool {
  ...
  constructor(toolCallId: string) {
    super(
      toolCallId,
      DeployWorkflowClientTool.id,
      DeployWorkflowClientTool.metadata
    );
  }
  ...
  getInterruptDisplays(): BaseClientToolMetadata["interrupt"] | undefined {
    ...
    const workflowId =
      params?.workflowId || useWorkflowRegistry.getState().activeWorkflowId;
  ...
  static readonly metadata: BaseClientToolMetadata = {
    displayNames: {
      ...
      [ClientToolCallState.success]: {
        text: "Deployed workflow",
        icon: Rocket,
      },
      [ClientToolCallState.error]: {
        text: "Failed to deploy workflow",
        icon: X,
      },
    ...
    getDynamicText: (params, state) => {
      ...
      const displayActionCapitalized = isChatDeploy
        ? "Deploy as chat"
        : actionCapitalized;
  ...
  private openDeployModal(tab: "api" | "chat" = "api"): void {
    window.dispatchEvent(
      new CustomEvent("open-deploy-modal", { detail: { tab } })
    );
  }
  ...
  async handleAccept(args?: DeployWorkflowArgs): Promise<void> {
    ...
    try {
      ...
      const res = await fetch(endpoint, {
        ...
        body:
          action === "deploy"
            ? JSON.stringify({ deployChatEnabled: false })
            : undefined,
      });
      ...
      if (action === "deploy") {
        ...
        successMessage =
          "Workflow deployed successfully. You can now call it via the API.";
      ...
      await this.markToolComplete(
        500,
        e?.message || "Failed to deploy/undeploy workflow"
      );
    }
  }
