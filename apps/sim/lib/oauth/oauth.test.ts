...
describe("OAuth Token Refresh", () => {
      ...
      it.concurrent(
        ...
        async () => {
          ...
          await withMockFetch(mockFetch, () =>
            refreshOAuthToken(providerId, refreshToken)
          );
  ...
  describe("Body Credential Providers", () => {
    ...
    it.concurrent(
      "should include Accept header for GitHub requests",
      async () => {
        const mockFetch = createMockFetch();
        const refreshToken = "test_refresh_token";

        await withMockFetch(mockFetch, () =>
          refreshOAuthToken("github", refreshToken)
        );
    ...
    it.concurrent(
      "should include User-Agent header for Reddit requests",
      async () => {
        const mockFetch = createMockFetch();
        const refreshToken = "test_refresh_token";

        await withMockFetch(mockFetch, () =>
          refreshOAuthToken("reddit", refreshToken)
        );

        const [, requestOptions] = mockFetch.mock.calls[0];
        expect(requestOptions.headers["User-Agent"]).toBe(
          "sim-studio/1.0 (https://github.com/)"
        );
      }
    );
  });
  ...
  describe("Token Response Handling", () => {
    it.concurrent(
      "should handle providers that return new refresh tokens",
      async () => {
    ...
    it.concurrent(
      "should use original refresh token when new one is not provided",
      async () => {
    ...
    it.concurrent(
      "should return null when access token is missing",
      async () => {
        const refreshToken = "test_refresh_token";
    ...
    it.concurrent(
      "should use default expiration when not provided",
      async () => {
        const refreshToken = "test_refresh_token";
