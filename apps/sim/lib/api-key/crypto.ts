...
export async function decryptApiKey(
  ...
  if (!ivHex || !encrypted || !authTagHex) {
    throw new Error(
      'Invalid encrypted API key format. Expected "iv:encrypted:authTag"'
    );
  }
...
export function isLegacyApiKeyFormat(apiKey: string): boolean {
  return apiKey.startsWith("sim_") && !apiKey.startsWith("sk-eab-");
}
