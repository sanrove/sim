...
export function buildPostMetadata(post: BlogMeta): Metadata {
  ...
  return {
    ...
    keywords: post.tags,
    authors: (post.authors && post.authors.length > 0
      ? post.authors
      : [post.author]
    ).map((a) => ({
      name: a.name,
    ...
    twitter: {
      ...
      description: post.description,
      images: [post.ogImage],
      creator: post.author.url?.includes("x.com")
        ? `@${post.author.xHandle || ""}`
        : undefined,
      site: "@simdotai",
    },
...
export function buildArticleJsonLd(post: BlogMeta) {
  return {
    ...
    publisher: {
      ...
      logo: {
        "@type": "ImageObject",
        url: "https://ethana.ai/logo/primary/medium.png",
      },
    },
    mainEntityOfPage: {
      "@type": "WebPage",
