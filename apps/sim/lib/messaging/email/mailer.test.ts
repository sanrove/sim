...
vi.mock("@/lib/core/config/env", () => ({
  env: {
    ...
    NEXT_PUBLIC_APP_URL: "https://test.ethana.ai",
    FROM_EMAIL_ADDRESS: "Sim <noreply@ethana.ai>",
  },
}));

vi.mock("@/lib/core/utils/urls", () => ({
  getEmailDomain: vi.fn().mockReturnValue("ethana.ai"),
  getBaseUrl: vi.fn().mockReturnValue("https://test.ethana.ai"),
}));
  ...
  describe("sendEmail", () => {
    ...
    it("should send a marketing email with unsubscribe headers", async () => {
      ...
      expect(mockSend).toHaveBeenCalledWith({
        ...
        headers: {
          "List-Unsubscribe":
            "<https://test.ethana.ai/unsubscribe?token=mock-token-123&email=test%40example.com>",
    ...
    it.concurrent(
      "should handle Resend API errors and fallback to Azure",
      async () => {
        // Mock Resend to fail
        mockSend.mockResolvedValue({
          data: null,
    ...
    it.concurrent(
      "should handle unexpected errors and fallback to Azure",
      async () => {
  ...
  describe("Azure Communication Services fallback", () => {
    it("should fallback to Azure when Resend fails", async () => {
      ...
      expect(mockAzureBeginSend).toHaveBeenCalledWith({
        senderAddress: "noreply@ethana.ai",
        content: {
          subject: testEmailOptions.subject,
          html: testEmailOptions.html,
        },
    ...
    it("should handle Azure Communication Services failure", async () => {
      ...
      expect(result.message).toBe(
        "Both Resend and Azure Communication Services failed"
      );
