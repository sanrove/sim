...
import {
  handleChargeDispute,
  handleDisputeClosed,
} from "@/lib/billing/webhooks/disputes";
...
export const auth = betterAuth({
  ...
  emailAndPassword: {
    ...
    sendResetPassword: async ({ user, url, token }, request) => {
      ...
      if (!result.success) {
        throw new Error(
          `Failed to send reset password email: ${result.message}`
        );
      }
    },
  },
      ...
      if (
        ...
        if (requestEmail) {
          ...
          if (env.ALLOWED_LOGIN_EMAILS) {
            const allowedEmails = env.ALLOWED_LOGIN_EMAILS.split(",").map(
          ...
          if (!isAllowed && env.ALLOWED_LOGIN_DOMAINS) {
            const allowedDomains = env.ALLOWED_LOGIN_DOMAINS.split(",").map(
    ...
    emailOTP({
      sendVerificationOTP: async (data: {
        ...
        try {
          ...
          if (
            !result.success &&
            result.message.includes("no email service configured")
          ) {
          ...
          getUserInfo: async (tokens) => {
            try {
              ...
              return {
                ...
                email:
                  data.email || `salesforce-${data.user_id}@salesforce.com`,
          ...
          getUserInfo: async (tokens) => {
            try {
              ...
              return {
                ...
                name:
                  profile.display_name || profile.username || "WordPress User",
              ...
              onSubscriptionUpdate: async ({
                ...
                if (resolvedSubscription.plan === "team") {
                  try {
                    ...
                    if (result.synced) {
                      logger.info(
                        "[onSubscriptionUpdate] Synced seat count from Stripe",
                        {
                          subscriptionId: resolvedSubscription.id,
                          referenceId: resolvedSubscription.referenceId,
                    ...
                    logger.error(
                      "[onSubscriptionUpdate] Failed to sync seat count",
                      {
                        subscriptionId: resolvedSubscription.id,
                        referenceId: resolvedSubscription.referenceId,
                        error,
