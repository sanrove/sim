...
function normalizeToolId(toolId: string): string {
  ...
  if (
    toolId.startsWith("workflow_executor_") &&
    toolId.length > "workflow_executor_".length
  ) {
...
function validateRequestBodySize(
  ...
  if (bodySize > MAX_REQUEST_BODY_SIZE_BYTES) {
    ...
    logger.error(
      `[${requestId}] Request body size exceeds limit for ${context}:`,
      {
        bodySize,
 ...
 */
function serializeExecutionContext(
...
function safeStringify(obj: any, requestId: string, context: string): string {
  try {
    ...
    if (error instanceof TypeError && error.message.includes("circular")) {
      logger.error(
        `[${requestId}] Circular reference detected in ${context}:`,
        {
          error: error.message,
...
async function processFileOutputs(
  ...
  try {
    ...
    const { FileToolProcessor } = await import(
...
export async function executeTool(
  ...
  try {
    ...
    if (isCustomTool(normalizedToolId)) {
      ...
      if (!tool) {
        logger.error(
          `[${requestId}] Custom tool not found: ${normalizedToolId}`
        );
      }
      ...
      if (!tool) {
        logger.error(
          `[${requestId}] Built-in tool not found: ${normalizedToolId}`
        );
      }
    }
    ...
    if (contextParams.credential) {
      ...
      try {
        ...
        logger.info(
          `[${requestId}] Fetching access token from ${baseUrl}/api/auth/oauth/token`
        );
        ...
        if (typeof window === "undefined") {
          try {
            const userId =
              contextParams._context?.userId || executionContext?.userId;
        ...
        logger.info(
          `[${requestId}] Successfully got access token for ${toolId}, length: ${
        ...
        logger.error(
          `[${requestId}] Error fetching access token for ${toolId}:`,
          {
        ...
        throw new Error(
          `Failed to obtain credential for tool ${toolId}: ${
    ...
    if (tool.directExecution) {
      ...
      if (tool.postProcess && result.success && !skipPostProcess) {
        try {
          finalResult = await tool.postProcess(
            result,
            contextParams,
            executeTool
          );
    ...
    const endpointUrl =
      typeof tool.request.url === "function"
        ? tool.request.url(contextParams)
        : tool.request.url;
  ...
  if (typeof window === "undefined") {
    if (isInternalRoute) {
      try {
        ...
        logger.error(
          `[${requestId}] Failed to generate internal token for ${context}:`,
          error
        );
      }
    } else {
      logger.info(
        `[${requestId}] Skipping internal auth token for external URL: ${context}`
      );
    }
  }
}
  ...
  try {
    ...
    if (isCustomTool(toolId) && tool.request.body) {
      ...
      if (
        ...
        try {
          ...
          logger.error(
            `[${requestId}] Custom tool validation failed for ${toolId}:`,
            {
              error:
    ...
    if (!response.ok) {
      ...
      try {
        ...
        logger.warn(
          `[${requestId}] Response is not JSON for ${toolId}, reading as text`
        );
        try {
          errorData = await response.text();
        } catch (textError) {
          logger.error(
            `[${requestId}] Failed to read response body for ${toolId}`
          );
          errorData = null;
        }
      }
      ...
      const errorToTransform = createTransformedErrorFromErrorInfo(
        errorInfo,
        tool.errorExtractor
      );
    ...
    if (status === 202) {
      ...
      if (tool.transformResponse) {
        ...
        try {
          ...
          throw new Error(
            `Failed to parse response from ${toolId}: ${jsonError}`
          );
        }
      }
    }
...
async function handleProxyRequest(
  ...
  try {
    ...
    if (!response.ok) {
      ...
      if (response.status === 413) {
        logger.error(
          `[${requestId}] Request body too large for proxy:${toolId} (HTTP 413)`
        );
        throw new Error(BODY_SIZE_LIMIT_ERROR_MESSAGE);
      }
  ...
  try {
    ...
    if (typeof window === "undefined") {
      try {
        ...
        logger.error(
          `[${actualRequestId}] Failed to generate internal token:`,
          error
        );
      }
    }
    ...
    if (params.arguments) {
      if (typeof params.arguments === "string") {
        try {
          ...
          logger.warn(
            `[${actualRequestId}] Failed to parse MCP arguments JSON:`,
            params.arguments
          );
          toolArguments = {};
        }
      ...
      toolArguments = Object.fromEntries(
        Object.entries(params).filter(
          ([key]) => !MCP_SYSTEM_PARAMETERS.has(key)
        )
      );
    }

    const workspaceId =
      params._context?.workspaceId || executionContext?.workspaceId;
    const workflowId =
      params._context?.workflowId || executionContext?.workflowId;
    const tenantId = params._context?.tenantId || executionContext?.tenantId;

    logger.info("[Tools] Extracted context for MCP tool request", {
      ...
      executionContextTenantId: executionContext?.tenantId || "none",
      paramsContextTenantId: params._context?.tenantId || "none",
    });
    ...
    const requestBody: Record<string, any> = {
      ...
      tenantId, // Pass tenantId for direct database resolution
    };
    ...
    logger.info(
      `[${actualRequestId}] Making MCP tool request to ${toolName} on ${serverId}`,
      {
        hasWorkspaceId: !!workspaceId,
        hasWorkflowId: !!workflowId,
    ...
    if (!response.ok) {
      ...
      if (response.status === 413) {
        logger.error(
          `[${actualRequestId}] Request body too large for mcp:${toolId} (HTTP 413)`
        );
        return {
          success: false,
          output: {},
    ...
    logger.info(
      `[${actualRequestId}] MCP tool ${toolId} executed successfully`
    );
    ...
    const errorMessage =
      error instanceof Error
        ? error.message
        : `Failed to execute MCP tool ${toolId}`;
