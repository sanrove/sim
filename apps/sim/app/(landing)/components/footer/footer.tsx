...
import {
  FOOTER_BLOCKS,
  FOOTER_TOOLS,
} from "@/app/(landing)/components/footer/consts";
...
export default function Footer({ fullWidth = false }: FooterProps) {
  return (
    <footer
      className={`${inter.className} relative w-full overflow-hidden bg-white`}
    >
      <div
        className={
          fullWidth
      ...
      >
        <div className={`flex gap-[80px] ${fullWidth ? "justify-center" : ""}`}>
          ...
          {/* Links section */}
          <div>
            <h2 className="mb-[16px] font-medium text-[14px] text-foreground">
              More Sim
            </h2>
          ...
          <div className="hidden sm:block">
            ...
            <div className="flex flex-col gap-[12px]">
              {FOOTER_BLOCKS.map((block) => (
                <Link
                  key={block}
                  href={`https://docs.ethana.ai/blocks/${block
                    .toLowerCase()
                    .replaceAll(" ", "-")}`}
                  target="_blank"
                  rel="noopener noreferrer"
          ...
          <div className="hidden sm:block">
            ...
            <div className="flex gap-[80px]">
              {/* First column */}
              <div className="flex flex-col gap-[12px]">
                {FOOTER_TOOLS.slice(0, Math.ceil(FOOTER_TOOLS.length / 4)).map(
                  (tool) => (
                    <Link
                      key={tool}
                      href={`https://docs.ethana.ai/tools/${tool
                        .toLowerCase()
                        .replace(/\s+/g, "_")}`}
                      target="_blank"
                      rel="noopener noreferrer"
