  ...
  try {
    ...
    try {
      ...
      const { hasAccess, workflow: workflowRecord } =
        await checkWorkflowAccessForChatCreation(
          workflowId,
          session.user.id,
          tenantDb
        );
      ...
      logger.info(
        `${
          workflowRecord.isDeployed ? "Redeployed" : "Auto-deployed"
        } workflow ${workflowId} for chat (v${result.version})`
      );
      ...
      const mergedCustomizations = {
        ...(customizations || {}),
        primaryColor:
          customizations?.primaryColor || "var(--brand-primary-hover-hex)",
        welcomeMessage:
      ...
      await database.insert(chat).values({
        ...
        allowedEmails:
          authType === "email" || authType === "sso" ? allowedEmails : [],
        outputConfigs,
        createdAt: new Date(),
        updatedAt: new Date(),
      });
      ...
      try {
        ...
        if (isDev) {
          ...
          chatUrl = `https://ethana.ai/chat/${identifier}`;
        }
      }
