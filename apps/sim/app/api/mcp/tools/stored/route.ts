...
export const GET = withMcpAuth("read")(
  async (request: NextRequest, { userId, workspaceId, requestId }) => {
    try {
      logger.info(
        `[${requestId}] Fetching stored MCP tools for workspace ${workspaceId}`
      );
      ...
      const workflows: { id: string; name: string }[] = await dbToUse
        .select({
          id: workflow.id,
          name: workflow.name,
        })
      ...
      const agentBlocks = await dbToUse
        .select({
      ...
      for (const block of agentBlocks) {
        ...
        const toolsSubBlock = subBlocks.tools as
      ...
      return createMcpErrorResponse(
        error instanceof Error
          ? error
          : new Error("Failed to fetch stored MCP tools"),
