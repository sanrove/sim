...
export const GET = withMcpAuth("read")(
  async (request: NextRequest, { userId, workspaceId, requestId }) => {
    try {
      logger.info(
        `[${requestId}] Listing MCP servers for workspace ${workspaceId}`
      );
      ...
      return createMcpErrorResponse(
        error instanceof Error
          ? error
          : new Error("Failed to list MCP servers"),
...
export const POST = withMcpAuth("write")(
  async (request: NextRequest, { userId, workspaceId, requestId }) => {
    try {
      ...
      const [existingServer] = await dbToUse
      ...
      return createMcpErrorResponse(
        error instanceof Error
          ? error
          : new Error("Failed to register MCP server"),
...
export const DELETE = withMcpAuth("admin")(
  async (request: NextRequest, { userId, workspaceId, requestId }) => {
    try {
      ...
      const [deletedServer] = await dbToUse
        .delete(mcpServers)
        .where(
          and(
            eq(mcpServers.id, serverId),
      ...
      logger.info(
        `[${requestId}] Successfully deleted MCP server: ${serverId}`
      );
