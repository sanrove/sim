...
describe("Function Execute API Route", () => {
  ...
  describe("Security Tests", () => {
    it.concurrent(
      "should use isolated-vm for secure sandboxed execution",
      async () => {
    ...
    it.concurrent(
      "should prevent VM escape via constructor chain",
      async () => {
        const req = createMockRequest("POST", {
    ...
    it.concurrent(
      "should prevent access to require via constructor chain",
      async () => {
        const req = createMockRequest("POST", {
          code: `
    ...
    it.concurrent(
      "should block SSRF attacks through secure fetch wrapper",
      async () => {
        expect(
          validateProxyUrl("http://169.254.169.254/latest/meta-data/").isValid
        ).toBe(false);
        expect(validateProxyUrl("http://127.0.0.1:8080/admin").isValid).toBe(
          false
        );
        expect(validateProxyUrl("http://192.168.1.1/config").isValid).toBe(
          false
        );
        expect(validateProxyUrl("http://10.0.0.1/internal").isValid).toBe(
          false
        );
      }
    );
    ...
    it.concurrent("should allow legitimate external URLs", async () => {
      expect(validateProxyUrl("https://api.github.com/user").isValid).toBe(
        true
      );
    ...
    it.concurrent("should block dangerous protocols", async () => {
      ...
      expect(validateProxyUrl("ftp://internal.server/files").isValid).toBe(
        false
      );
  ...
  describe("Basic Function Execution", () => {
    it.concurrent(
      "should execute simple JavaScript code successfully",
      async () => {
        const req = createMockRequest("POST", {
    ...
    it.concurrent(
      "should return computed result for multi-line code",
      async () => {
        const req = createMockRequest("POST", {
          code: "const a = 1;\nconst b = 2;\nconst c = 3;\nconst d = 4;\nreturn a + b + c + d;",
          timeout: 5000,
  ...
  describe("Template Variable Resolution", () => {
    it.concurrent(
      "should resolve environment variables with {{var_name}} syntax",
      async () => {
        const req = createMockRequest("POST", {
          code: "return {{API_KEY}}",
          envVars: {
    ...
    it.concurrent(
      "should resolve tag variables with <tag_name> syntax",
      async () => {
        const req = createMockRequest("POST", {
          code: "return <email>",
          params: {
    ...
    it.concurrent(
      "should NOT treat email addresses as template variables",
      async () => {
        const req = createMockRequest("POST", {
          code: 'return "Email sent to user"',
          params: {
            email: {
              from: "John Doe <john@ethana.ai>",
    ...
    it.concurrent(
      "should only match valid variable names in angle brackets",
      async () => {
        const req = createMockRequest("POST", {
      ...
      async () => {
        const gmailData = {
          ...
          rawEmail: {
            ...
            payload: {
              headers: [
                { name: "From", value: "John Doe <john@ethana.ai>" },
      ...
      async () => {
        const complexEmailData = {
          email: {
            ...
            bodyHtml:
              "<div>HTML content with \"quotes\" and 'apostrophes'</div>",
            bodyText: "Text with\nnewlines\tand\ttabs",
          },
        };
  ...
  describe("Custom Tools", () => {
    it.concurrent(
      "should handle custom tool execution with direct parameter access",
      async () => {
        const req = createMockRequest("POST", {
  ...
  describe("Security and Edge Cases", () => {
    it.concurrent("should handle malformed JSON in request body", async () => {
      const req = new NextRequest(
        "http://localhost:5863/api/function/execute",
        {
          method: "POST",
          body: "invalid json{",
  ...
  describe("Enhanced Error Handling", () => {
    ...
    it.concurrent(
      "should provide helpful suggestions for common syntax errors",
      async () => {
        const req = createMockRequest("POST", {
