...
const CareersSubmissionSchema = z.object({
  ...
  position: z
    .string()
    .min(2, "Please specify the position you are interested in"),
  linkedin: z
    .string()
...
export async function POST(request: NextRequest) {
  ...
  try {
      ...
      return NextResponse.json(
        {
          ...
          errors: [
            { path: ["resume"], message: "File size must be less than 10MB" },
          ],
        },
        { status: 400 }
      );
      ...
      return NextResponse.json(
        {
          ...
          errors: [
            { path: ["resume"], message: "File must be PDF or Word document" },
          ],
        },
        { status: 400 }
      );
    ...
    if (!careersEmailResult.success) {
      logger.error(`[${requestId}] Failed to send email to careers@ethana.ai`, {
    ...
    if (!confirmationResult.success) {
      logger.warn(
        `[${requestId}] Failed to send confirmation email to applicant`,
        {
          email: validatedData.email,
    ...
    if (error instanceof z.ZodError) {
      logger.warn(`[${requestId}] Invalid application data`, {
        errors: error.errors,
      });
      return NextResponse.json(
        {
    ...
    return NextResponse.json(
      {
        success: false,
        message:
          "Failed to submit application. Please try again or email us directly at careers@ethana.ai",
      },
      { status: 500 }
    );
  }
