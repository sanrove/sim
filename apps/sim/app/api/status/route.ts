...
import type {
  IncidentIOWidgetResponse,
  StatusResponse,
  StatusType,
} from "@/app/api/status/types";
...
export async function GET() {
  try {
    ...
    const response = await fetch("https://status.ethana.ai/api/v1/summary", {
      method: "GET",
      headers: {
    ...
    const statusResponse: StatusResponse = {
      status,
      message,
      url: data.page_url || "https://status.ethana.ai",
