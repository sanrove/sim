  ...
  const pages = useMemo(
    (): PageItem[] => [
      ...
      },
      {
        ...
        name: "Docs",
        icon: BookOpen,
        href: brand.documentationUrl || "https://docs.ethana.ai/",
      },
    ],
        ...
        case "tool":
          if (item.blockType) {
            const enableTriggerMode =
              item.type === "trigger" && item.config
                ? hasTriggerCapability(item.config)
                : false;
        ...
        case "workflow":
          if (!item.isCurrent && item.href) {
            ...
            window.dispatchEvent(
              new CustomEvent(SIDEBAR_SCROLL_EVENT, {
      ...
      switch (e.key) {
        case "ArrowDown":
          ...
          setSelectedIndex((prev) =>
            Math.min(prev + 1, displayedItemsInVisualOrder.length - 1)
          );
          break;
        case "ArrowUp":
          e.preventDefault();
  ...
  useEffect(() => {
    if (open && selectedIndex >= 0) {
      const element = document.querySelector(
        `[data-search-item-index="${selectedIndex}"]`
      );
      if (element) {
                  ...
                  <div key={type} className="mb-[10px] last:mb-0">
                    ...
                    <div className="space-y-[2px]">
                      {items.map((item, itemIndex) => {
                        ...
                        const visualIndex =
                          displayedItemsInVisualOrder.indexOf(item);
