...
export function UsageLimitActions() {
  ...
  const limitOptions = LIMIT_INCREMENTS.map(
    (increment) => baseLimit + increment
  );
  ...
  const handleNavigateToUpgrade = () => {
    if (isHosted) {
      ...
      window.open("https://www.ethana.ai", "_blank");
    }
  };
  ...
  return (
    <>
      {limitOptions.map((limit) => {
        const isLoading =
          updateUsageLimitMutation.isPending && selectedAmount === limit;
