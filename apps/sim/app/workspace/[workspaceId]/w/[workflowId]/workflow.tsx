...
} from "@/lib/workflows/blocks/block-dimensions";
...
const LazyChat = lazy(() =>
  import(
    "@/app/workspace/[workspaceId]/w/[workflowId]/components/chat/chat"
  ).then((mod) => ({
    default: mod.Chat,
  }))
);
...
const WorkflowContent = React.memo(() => {
  ...
  const { workflows, activeWorkflowId, hydration, setActiveWorkflow } =
    useWorkflowRegistry(
      useShallow((state) => ({
  ...
  const { updateNodeDimensions, setDragStartPosition, getDragStartPosition } =
    useWorkflowStore(
      useShallow((state) => ({
  ...
  const { applyAutoLayoutAndUpdateStore } = useAutoLayout(
  ...
  useEffect(() => {
    ...
    window.addEventListener(
      "open-oauth-connect",
      handleOpenOAuthConnect as EventListener
    );
    return () =>
  ...
  const edgesForDisplay = useMemo(() => {
    ...
    if (
      !isShowingDiff &&
      isDiffReady &&
      diffAnalysis?.edge_diff?.deleted_edges
    ) {
        ...
        if (parts.length >= 4) {
          ...
          if (sourceEndIndex > 0 && targetStartIndex > 0) {
            ...
            const targetId = parts
              .slice(targetStartIndex, parts.length - 1)
              .join("-");
    ...
    return edgesToFilter.filter((edge) => {
      ...
      return (
        !isAnnotationOnlyBlock(sourceBlock.type) &&
        !isAnnotationOnlyBlock(targetBlock.type)
      );
    });
  ...
  const updateNodeParent = useCallback(
    (nodeId: string, newParentId: string | null, affectedEdges: any[] = []) => {
      ...
      if (!affectedEdges.length && !newParentId && oldParentId) {
        affectedEdges = edgesForDisplay.filter(
  ...
  const removeEdgesForNode = useCallback(
    (blockId: string, edgesToRemove: Edge[]): void => {
      ...
      window.dispatchEvent(
        new CustomEvent("skip-edge-recording", { detail: { skip: true } })
      );
      ...
      try {
        ...
        window.dispatchEvent(
          new CustomEvent("skip-edge-recording", { detail: { skip: false } })
        );
      }
    },
    [removeEdge]
  ...
  useEffect(() => {
    ...
    window.addEventListener(
      "remove-from-subflow",
      handleRemoveFromSubflow as EventListener
    );
    return () =>
      window.removeEventListener(
        "remove-from-subflow",
    ...
    (
      ...
      return candidateBlocks
        ...
        .map((b) => ({
          block: b,
          distance: Math.sqrt(
            (b.position.x - targetPosition.x) ** 2 +
              (b.position.y - targetPosition.y) ** 2
          ),
        }))
  ...
  const tryCreateAutoConnectEdge = useCallback(
    (
      ...
      if (
        options.blockType === "starter" ||
        isAnnotationOnlyBlock(options.blockType)
      ) {
        return undefined;
      }
      ...
      if (
        options.existingChildBlocks &&
        options.existingChildBlocks.length > 0
      ) {
        const closestBlock = findClosestBlockInSet(
          options.existingChildBlocks,
          position
        );
        if (closestBlock) {
  ...
  const handleToolbarDrop = useCallback(
    (
      ...
      try {
        ...
        const defaultTriggerNameDrop = TriggerUtils.getDefaultTriggerName(
          data.type
        );
        ...
        if (containerInfo) {
          ...
          const autoConnectEdge = tryCreateAutoConnectEdge(
            relativePosition,
            id,
            {
              blockType: data.type,
  ...
  useEffect(() => {
    const handleAddBlockFromToolbar = (event: CustomEvent) => {
      ...
      const calculateSmartPosition = (): { x: number; y: number } => {
        ...
        const rootBlocks = Object.values(blocks).filter(
          (b) => !b.data?.parentId
        );
  ...
  useEffect(() => {
    const handleOverlayToolbarDrop = (event: Event) => {
      ...
      try {
        ...
        if (!canvasElement) {
          logger.warn(
            "Workflow canvas element not found for overlay toolbar drop"
          );
          return;
        }
        ...
        logger.error(
          "Error handling toolbar drop from empty-workflow overlay",
          { err }
        );
      }
    };
      ...
      const message =
        type === "trigger_in_subflow"
          ...
          : `A workflow can only have one ${
              triggerName || "trigger"
            } trigger block. Please remove the existing one before adding a new one.`;
      addNotification({
        level: "error",
        message,
        workflowId: activeWorkflowId || undefined,
  ...
  const onDragOver = useCallback(
    (event: React.DragEvent) => {
      ...
      try {
        ...
        const isSubflowDrag =
          document.body.classList.contains("sim-drag-subflow");
  ...
  useEffect(() => {
    ...
    if (
      ...
      hydration.workflowId === currentId
    ) {
      return;
    }
    ...
    const hasLoadError =
      hydration.phase === "error" && hydration.workflowId === currentId;
  ...
  useEffect(() => {
    ...
    if (workflowData && workflowData.workspaceId !== workspaceId) {
      ...
      router.replace(
        `/workspace/${workflowData.workspaceId}/w/${workflowIdParam}`
      );
    }
  }, [
    workflowIdParam,
  ...
  const blocksStructureHash = useMemo(() => {
    ...
    const hash = Object.values(blocks)
      .map((b) => {
        ...
        return `${b.id}:${b.type}:${b.name}:${b.height}:${
          b.data?.parentId || ""
        }:${width}:${height}`;
      })
      .join("|");
  ...
  const derivedNodes = useMemo(() => {
    ...
    Object.entries(blocks).forEach(([blockId, block]) => {
      ...
      nodeArray.push({
        ...
        height: Math.max(
          block.height || BLOCK_DIMENSIONS.MIN_HEIGHT,
    ...
    (draggedNodeId: string, draggedNodePosition: { x: number; y: number }) => {
      ...
      setDisplayNodes((currentNodes) => {
        ...
        childNodes.forEach((node) => {
          const nodePosition =
            node.id === draggedNodeId ? draggedNodePosition : node.position;
          const { width: nodeWidth, height: nodeHeight } = getBlockDimensions(
            node.id
          );
      ...
      if (intersectingNodes.length > 0) {
        ...
        if (containerElement) {
          ...
          if (
            ...
            (bestContainerMatch.container.data as SubflowNodeData)?.kind ===
              "loop"
          ) {
            containerElement.classList.add("loop-node-drag-over");
          } else if (
    ...
    (_event: React.MouseEvent, node: any) => {
      ...
      if (currentParentId) {
        ...
        if (parentNode) {
          const containerDimensions = {
            ...
            height:
              parentNode.data?.height || CONTAINER_DIMENSIONS.DEFAULT_HEIGHT,
          };
      ...
      if (potentialParentId) {
        ...
        if (block && TriggerUtils.isTriggerBlock(block)) {
          ...
          logger.warn(
            "Prevented trigger block from being placed inside a container",
            {
              blockId: node.id,
      ...
      if (potentialParentId) {
        ...
        // Account for header (50px), left padding (16px), and top padding (16px)
        const containerAbsPosBefore =
          getNodeAbsolutePosition(potentialParentId);
  ...
  const handleEdgeDelete = useCallback(
    (edgeId: string) => {
      removeEdge(edgeId);
      setSelectedEdgeInfo((current) =>
        current?.id === edgeId ? null : current
      );
    },
    [removeEdge]
  );
  ...
  return (
    <div className="flex h-full w-full flex-col overflow-hidden bg-[var(--bg)]">
      <div className="relative h-full w-full flex-1 bg-[var(--bg)]">
        ...
        <div
          className={`absolute inset-0 z-[5] flex items-center justify-center bg-[var(--bg)] transition-opacity duration-150 ${
            isWorkflowReady ? "pointer-events-none opacity-0" : "opacity-100"
          }`}
        >
          <div
            className={`h-[18px] w-[18px] rounded-full ${
              isWorkflowReady ? "" : "animate-spin"
            }`}
            style={{
              background:
        ...
        {isWorkflowReady && (
          <>
            ...
            <ReactFlow
              ...
              onConnectStart={
                effectivePermissions.canEdit ? onConnectStart : undefined
              }
              onConnectEnd={
                effectivePermissions.canEdit ? onConnectEnd : undefined
              }
              nodeTypes={nodeTypes}
              edgeTypes={edgeTypes}
              ...
              // className={`workflow-container h-full bg-[var(--bg)] transition-opacity duration-150 ${
              //   isCanvasReady ? "opacity-100" : "opacity-0"
              // }`}
              onNodeDrag={effectivePermissions.canEdit ? onNodeDrag : undefined}
              onNodeDragStop={
                effectivePermissions.canEdit ? onNodeDragStop : undefined
              }
              onNodeDragStart={
                effectivePermissions.canEdit ? onNodeDragStart : undefined
              }
