...
Öffnen Sie [http://localhost:5863](http://localhost:5863)
...
server {
    ...
    location / {
        proxy_pass http://127.0.0.1:5863;
    ...
    location /socket.io/ {
        proxy_pass http://127.0.0.1:5865;
        proxy_http_version 1.1;
        proxy_set_header Upgrade $http_upgrade;
        proxy_set_header Connection "upgrade";
    }
...
<Callout type="warning">
  Innerhalb von Docker bezieht sich `localhost` auf den Container, nicht auf
  Ihren Host. Verwenden Sie `host.docker.internal` oder die IP-Adresse Ihres
  Hosts.
</Callout>
