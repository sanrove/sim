...
<Callout type="info">
  KI-Modellpreise werden pro Million Token berechnet. Die Berechnung teilt durch
  1.000.000, um die tatsächlichen Kosten zu ermitteln. Workflows ohne KI-Blöcke
  verursachen nur die Basis-Ausführungsgebühr.
</Callout>
...
<Callout type="warning">
  Die angezeigten Preise entsprechen den Tarifen vom 10. September 2025.
  Überprüfen Sie die Dokumentation der Anbieter für aktuelle Preise.
</Callout>
...
```bash
curl -X GET -H "X-API-Key: YOUR_API_KEY" -H "Content-Type: application/json" https://ethana.ai/api/users/me/usage-limits
```
...
- Überprüfen Sie Ihre aktuelle Nutzung unter [Einstellungen → Abonnement](https://ethana.ai/settings/subscription)
