...
Copilot はエディター内のアシスタントで、Sim Copilot を使用してワークフローの構築や編集を支援し、それらを理解して改善するのに役立ちます。以下のことができます：

- **説明**: Sim や現在のワークフローに関する質問に回答
- **ガイド**: 編集の提案やベストプラクティスの提示
- **編集**: 承認後にブロック、接続、設定の変更を実行

<Callout type="info">
  CopilotはSimが管理するサービスです。セルフホスト型のデプロイメントでは、ホスト型アプリ（ethana.ai
  → 設定 → Copilot）でCopilot APIキーを生成してください 1.
  [ethana.ai](https://ethana.ai) → 設定 → Copilotに移動し、Copilot APIキーを生成
  2. セルフホスト環境の `COPILOT_API_KEY` にその値を設定
</Callout>

## コンテキストメニュー (@)

`@` 記号を使用して、さまざまなリソースを参照し、ワークスペースについて Copilot により多くのコンテキストを提供できます：
...
Copilot インターフェースのモードセレクターを使用して、異なる推論モード間を簡単に切り替えることができます：
...
- **MAX モードの有効化**：最も徹底的な分析が必要な場合に最大限の推論能力を切り替え
...
Copilot の使用料金は、基盤となる LLM からのトークンごとに請求されます：

- **入力トークン**：プロバイダーの基本料金で請求（**原価**）
- **出力トークン**：プロバイダーの基本出力料金の**1.5 倍**で請求
...
<Callout type="info">
  モデル価格は100万トークンあたりの料金です。実際のコストを算出するには1,000,000で割ります。背景と例については
  <a href="/execution/costs">コスト計算ページ</a>をご覧ください。
</Callout>
