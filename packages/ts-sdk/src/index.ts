...
export class SimStudioClient {
  ...
  constructor(config: SimStudioConfig) {
    ...
    this.baseUrl = normalizeBaseUrl(config.baseUrl || "https://ethana.ai");
  }
  ...
  async executeWithRetry(
    ...
    for (let attempt = 0; attempt <= maxRetries; attempt++) {
      try {
        ...
        if (
          !(error instanceof SimStudioError) ||
    ...
    throw (
      lastError ||
      new SimStudioError("Max retries exceeded", "MAX_RETRIES_EXCEEDED")
    );
  }
  ...
  async getUsageLimits(): Promise<UsageLimits> {
    ...
    try {
      ...
      throw new SimStudioError(
        error?.message || "Failed to get usage limits",
        "USAGE_ERROR"
      );
    }
  }
